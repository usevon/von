...
<Properties>
  ...
  <Property name="status" type="string">
    Endpoint status: `active`, `paused`, or `disabled`. Defaults to `active`.
  </Property>
  <Property name="maxAttempts" type="number">
    Maximum delivery attempts including the first send (1-10). Defaults to 4.
  </Property>
...
When a delivery fails, Von retries with exponential backoff from a 1 second base delay:

1. First retry: 1s
2. Second retry: 2s
3. Third retry: 4s
4. Then continues doubling until `maxAttempts` is reached
