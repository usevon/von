...
const scoreDocument = (doc: SearchDocument, terms: string[], fullQuery: string) => {
  ...
  if (title.includes(full)) score += 120;
  if (href.includes(full)) score += 60;
  if (section.includes(full)) score += 40;
  if (content.includes(full)) score += 20;

  for (const term of terms) {
    if (title.startsWith(term)) score += 30;
    if (title.includes(term)) score += 20;
    if (href.includes(term)) score += 12;
