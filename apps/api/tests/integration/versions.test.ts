...
describe.skipIf(!getApiKey())("Versions CRUD", () => {
  ...
  test("GET /versions/:version returns version", async () => {
    ...
    const { data, error } = await client.versions({ version: testVersion }).get({
      headers: { authorization: `Bearer ${apiKey}` },
    })
  ...
  test("PATCH /versions/:version updates version", async () => {
    ...
    const { data, error } = await client.versions({ version: testVersion }).patch(
      {
        transforms: {
          "product.updated": {
    ...
    if (error) throw error
    expect(data.transforms["product.updated"].rename).toEqual({ features: "newItems" })
  })

  test("DELETE /versions/:version deletes version", async () => {
    if (!createdVersion) return

    const { data, error } = await client.versions({ version: testVersion }).delete(null, {
      headers: { authorization: `Bearer ${apiKey}` },
    })
