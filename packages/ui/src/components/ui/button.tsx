...
const buttonVariants = tv({
  base: "relative inline-flex shrink-0 cursor-pointer select-none items-center justify-center gap-2 whitespace-nowrap rounded-none border font-medium text-base outline-none transition-[box-shadow,transform] before:pointer-events-none before:absolute before:inset-0 before:rounded-none pointer-coarse:after:absolute pointer-coarse:after:size-full pointer-coarse:after:min-h-11 pointer-coarse:after:min-w-11 focus-visible:ring-2 focus-visible:ring-ring focus-visible:ring-offset-1 focus-visible:ring-offset-background disabled:pointer-events-none disabled:opacity-64 sm:text-sm [&_svg:not([class*='opacity-'])]:opacity-80 [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:-mx-0.5 [&_svg]:shrink-0 [:active,[data-pressed]]:scale-[0.97]",
  defaultVariants: {
    size: "default",
    variant: "default",
  },
  variants: {
    ...
    variant: {
      ...
      ghost:
        "border-transparent hover:bg-accent data-pressed:bg-accent [:active,[data-pressed]]:scale-100",
      "ghost-light":
        "border-white/20 bg-white/10 text-white backdrop-blur-sm hover:bg-white/20 data-pressed:bg-white/25",
      "ghost-dark":
        "border-black/10 bg-black/5 text-zinc-900 backdrop-blur-sm hover:bg-black/10 data-pressed:bg-black/15",
      link: "border-transparent underline-offset-4 hover:underline [:active,[data-pressed]]:scale-100",
      outline:
        "border-[#dde1e6] bg-[#eff1f4] text-[#1e2430] shadow-none before:shadow-none hover:bg-[#e7ebf0] dark:border-white/14 dark:bg-white/8 dark:text-zinc-100 dark:hover:bg-white/12 [:active,[data-pressed]]:bg-[#dfe5ec] dark:[:active,[data-pressed]]:bg-white/16",
      secondary:
...
function Button({ className, variant, size, render, ...props }: ButtonProps) {
  ...
  const defaultProps = {
    className: cn(buttonVariants({ class: className, size, variant })),
    "data-slot": "button",
    type: typeValue,
  };
