...
export const requestDeviceCode = (clientId = "von-cli"): Promise<DeviceCodeResponse> => {
  return request("api", "/api/auth/device/code", {
    method: "POST",
...
export const getSession = (token: string): Promise<UserSession | null> => {
  return request<UserSession>("api", "/api/auth/get-session", { token }).catch(() => null)
}
...
export const listOrganizations = (token: string): Promise<Organization[]> => {
  return request<Organization[]>("api", "/api/auth/organization/list", { token }).catch(() => [])
}
...
export const registerTunnel = async (
  ...
  const config = loadConfig()
  const result = await request<{ tunnelId: string; secret: string }>("tunnel", "/register", {
    method: "POST",
    token,
  ...
  const wsUrl = config.tunnelUrl
    .replace("http://", "ws://")
    .replace("https://", "wss://") + `/ws/${result.tunnelId}`
...
export const rotateTunnel = async (
  ...
  return request<{ secret: string }>("tunnel", `/rotate/${tunnelId}`, {
    method: "POST",
    token,
  })
}
