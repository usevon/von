...
export const generateIdempotencyKey = (): string =>
  `von-${Date.now()}-${crypto.randomUUID()}`;
...
export const vonFetch = async <T>(
  ...
  const retryStrategy = options.retry
    ? createRetryStrategy(options.retry)
    : null;
  let attempt = 0;
  ...
  while (true) {
    try {
      ...
      if (
        retryStrategy &&
        (await retryStrategy.shouldAttemptRetry(attempt, response))
      ) {
        if (options.onRetry) {
      ...
      if (
        retryStrategy &&
        (await retryStrategy.shouldAttemptRetry(attempt, null))
      ) {
        if (options.onRetry && lastResponse) {
          await options.onRetry(
            { request: context, response: lastResponse },
            attempt
          );
        }
