...
export function shouldTransitionToHalfOpen(state: CircuitBreakerState): boolean {
  if (state.circuitState !== "open") return false
  if (!state.circuitOpenedAt) return false
...
export function getSuccessUpdate(): { circuitState: CircuitState; failureCount: number } {
  return {
    circuitState: "closed",
    failureCount: 0,
  }
