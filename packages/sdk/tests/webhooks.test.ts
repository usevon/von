...
describe("Webhooks Methods", () => {
  ...
  describe("sendBatch", () => {
    test("sends POST to /webhooks/batch", async () => {
      ...
      expect(fetchSpy).toHaveBeenCalledWith(
        "https://api.test.com/webhooks/batch",
        expect.objectContaining({
          method: "POST",
        })
      );
  ...
  describe("list", () => {
    test("sends GET to /webhooks/events", async () => {
      ...
      expect(fetchSpy).toHaveBeenCalledWith(
        "https://api.test.com/webhooks/events",
        expect.objectContaining({
          method: "GET",
        })
      );
    ...
    test("includes pagination params in query string", async () => {
      ...
      expect(fetchSpy).toHaveBeenCalledWith(
        "https://api.test.com/webhooks/events?limit=10&offset=20",
        expect.anything()
      );
    });
  });
  ...
  describe("get", () => {
    test("sends GET to /webhooks/events/:id", async () => {
      ...
      expect(fetchSpy).toHaveBeenCalledWith(
        "https://api.test.com/webhooks/events/evt_123",
        expect.objectContaining({
          method: "GET",
        })
      );
