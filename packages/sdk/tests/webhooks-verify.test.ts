...
describe("verifyWebhook", () => {
  ...
  test("verifies valid signature and returns parsed payload", () => {
    ...
    const result = verifyWebhook<{ event: string; data: { id: number } }>(
      payload,
      header,
      secret
    );
  ...
  test("throws on invalid signature", () => {
    ...
    expect(() => verifyWebhook(payload, header, secret)).toThrow(
  ...
  test("throws on missing timestamp in header", () => {
    ...
    expect(() => verifyWebhook(payload, `v1=${sig}`, secret)).toThrow(
      "Invalid signature header format"
    );
  });

  test("throws on missing signature in header", () => {
    expect(() => verifyWebhook(payload, "t=123", secret)).toThrow(
      "Invalid signature header format"
    );
  });
  ...
  test("throws on invalid JSON payload", () => {
    ...
    expect(() => verifyWebhook(invalidPayload, header, secret)).toThrow(
  ...
  test("throws on tampered payload", () => {
    ...
    expect(() => verifyWebhook(tamperedPayload, header, secret)).toThrow(
      "Invalid signature"
    );
  });
});
