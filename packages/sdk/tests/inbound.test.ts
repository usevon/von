  ...
  describe('list', () => {
    test('sends GET to /inbound', async () => {
      const inboundResponse = {
        inboundEndpoints: [
          { id: 'in_1', name: 'Stripe', provider: 'stripe', secret: 'insec_1', forwardUrl: 'https://myapp.com/webhook', enabled: true, createdAt: '2024-01-01T00:00:00Z', updatedAt: '2024-01-01T00:00:00Z' },
        ],
        total: 1,
      }
    ...
    test('includes pagination params in query string', async () => {
      ...
      expect(fetchSpy).toHaveBeenCalledWith('https://api.test.com/inbound?limit=10&offset=20', expect.anything())
    })
  })
