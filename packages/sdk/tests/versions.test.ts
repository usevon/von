...
describe('Versions Methods', () => {
  ...
  describe('create', () => {
    ...
    test('includes transforms with all mapping types', async () => {
      ...
      expect(body.transforms['product.updated'].rename).toEqual({ features: 'items', 'user.profile.name': 'userName' })
      expect(body.transforms['product.updated'].remove).toEqual(['internalField', 'debug.logs'])
      expect(body.transforms['product.updated'].defaults).toEqual({ legacyField: null, newField: 'default' })
    })
  })
  ...
  describe('list', () => {
    ...
    test('includes pagination params in query string', async () => {
      ...
      expect(fetchSpy).toHaveBeenCalledWith('https://api.test.com/versions?limit=10&offset=20', expect.anything())
    })
  })
