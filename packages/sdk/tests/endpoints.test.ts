  ...
  describe('list', () => {
    test('sends GET to /endpoints', async () => {
      const endpointsResponse = {
        endpoints: [
          { id: 'ep_1', url: 'https://example.com/webhook', secret: 'whsec_1', description: null, enabled: true, retryCount: 3, timeoutMs: 30000, createdAt: '2024-01-01T00:00:00Z', updatedAt: '2024-01-01T00:00:00Z' },
        ],
        total: 1,
      }
    ...
    test('includes pagination params in query string', async () => {
      ...
      expect(fetchSpy).toHaveBeenCalledWith('https://api.test.com/endpoints?limit=10&offset=20', expect.anything())
    })
  })
